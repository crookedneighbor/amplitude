...
describe('initialization', () => {
  context('with api token', () => {
    ...
    it('throws an error when no user_id or device_id is provided', () => {
      sandbox.spy(Amplitude);

      expect(() => {
        let amplitude = new Amplitude('token');
      }).to.throw(/Missing user_id or device_id/);
    });
  });
